...
@section('admin-title', 'Tambah Foto Galeri')
...
@section('admin-content')
  <div class="max-w-2xl mx-auto">
    <div class="mb-6 flex items-center gap-3">
      ...
      <h3 class="text-lg font-bold text-slate-800">Tambah Foto Baru</h3>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
      <form method="POST" action="{{ route('admin.galleries.store') }}" class="p-8" enctype="multipart/form-data">
        @csrf
        
        <div class="mb-6">
          <label class="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-2">Pilih Foto *</label>
          ...
          <p class="text-[11px] text-slate-400 mt-2">Format: JPG, PNG, WEBP (Maks 5MB). Rekomendasi: 16:9 atau 4:3.</p>
        </div>

        <div class="space-y-6">
          <div>
            <label class="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-2">Judul / Keterangan (Opsional)</label>
            <input type="text" name="title" placeholder="Misal: Sesi Pelatihan Bahasa" value="{{ old('title') }}"
        ...
        <div class="flex items-center justify-end gap-3">
          ...
          <button type="submit" class="px-8 py-2.5 bg-primary text-white rounded-xl text-sm font-bold shadow-lg shadow-primary/20 hover:bg-primary/90 transition-all flex items-center gap-2">
            <span class="material-symbols-outlined text-lg">save</span> Simpan Foto
          </button>
        </div>
      </form>
    </div>
