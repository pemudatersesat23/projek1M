...
@section('admin-title', isset($siswa) ? 'Edit Siswa' : 'Tambah Siswa')
...
@section('admin-content')
  ...
  <div class="bg-white rounded-xl border border-slate-200 shadow-sm">
    <div class="px-6 py-4 border-b border-slate-200">
      <h4 class="font-bold text-slate-800">{{ isset($siswa) ? 'Edit Data Siswa' : 'Tambah Data Siswa Baru' }}</h4>
    ...
    <form method="POST" action="{{ isset($siswa) ? route('admin.siswa.update', $siswa) : route('admin.siswa.store') }}" class="p-6">
      @csrf
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-6">
        ...
        <div>
          ...
          <input type="text" name="wa" placeholder="08xx-xxxx-xxxx" required
                 value="{{ old('wa', $siswa->wa ?? '') }}"
                 class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-primary focus:border-primary outline-none">
        </div>
        <div>
        ...
        <div>
          ...
          <input type="date" name="tgl_lahir"
                 value="{{ old('tgl_lahir', isset($siswa) && $siswa->tgl_lahir ? $siswa->tgl_lahir->format('Y-m-d') : '') }}"
                 class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-primary focus:border-primary outline-none">
        </div>
        <div>
        ...
        <div>
          <label class="block text-xs font-semibold uppercase tracking-wide text-slate-500 mb-2">Pendidikan Terakhir</label>
      ...
      @php $extra = isset($siswa) && $siswa->extra_fields ? json_decode($siswa->extra_fields, true) : []; @endphp
      ...
      <div class="mb-6">
        ...
        <textarea name="catatan" placeholder="Catatan tambahan..." rows="3"
                  class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-primary outline-none resize-y">{{ old('catatan', $siswa->catatan ?? '') }}</textarea>
      </div>
      ...
      <div class="flex items-center gap-3 pt-4 border-t border-slate-200">
        <button type="submit" class="px-6 py-2.5 bg-primary text-white rounded-lg text-sm font-medium hover:bg-primary/90 transition-colors flex items-center gap-2">
          ...
          {{ isset($siswa) ? 'Update Data' : 'Simpan Data Siswa' }}
        </button>
...
<script>
  function toggleExtraFields() {
    ...
    if (program === 'Engineering') {
      document.getElementById('fields-engineering').classList.remove('hidden');
    } else if (program === 'Tokutei Ginou (TG)') {
      document.getElementById('fields-tg').classList.remove('hidden');
    } else if (program === 'Returnee / Ex Jepang') {
      document.getElementById('fields-returnee').classList.remove('hidden');
    }
  }
  // Run on page load to show fields if program is already selected
