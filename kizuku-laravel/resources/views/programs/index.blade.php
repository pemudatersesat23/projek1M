        ...
        <div class="card-wrap">
            ...
            @foreach($programs as $index => $p)
                <article class="prog-card {{ $cardClasses[$index % 4] }} reveal" style="height: 100%; display: flex; flex-direction: column;">
                    ...
                    <div class="prog-footer" style="padding-top: 20px; border-top: 1px solid rgba(0,0,0,0.05);">
                        @php 
                        ...
                        @if($activeBatch)
                            <a class="btn btn-{{ $cardClasses[$index % 4] }}" href="{{ route('programs.show', $p->slug) }}" style="width: 100%; text-align: center;">
                                {{ __('messages.program.batch.enroll') }} {{ $activeBatch->nama_batch }}
                            </a>
                        @elseif($upcomingBatch)
                            <a class="btn btn-outline" href="{{ route('programs.show', $p->slug) }}" style="width: 100%; text-align: center;">
